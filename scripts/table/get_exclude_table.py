...
for name in data_type:
    ...
    outFile.write("\\caption{Result of 10-fold cross validation on " + name + " data set. The first column is the feature (s) we exclude. We divide the rows into 4 groups by the number of feature we exclude. In each column, we boldface the value which gives the best and worst cumulative percentage of mutation genes in the group.}\\medskip\n")
    outFile.write("\\label{exclude_" + name +"}\n")
    ...
    for idx, feature in enumerate(FEATURE):
        ...
        filename = "../../output/exclude/CV_" + name + "_e_" + feature + "/rank_" + name + ".csv"
        ...
        total_rank.append([counts[0], sum(counts[1:10]), sum(counts[10:99]), counts[99]])
