...
class Data:
    ...
    def bucketize_data(self):
        ...
        for entry in self.casedisgene:
            case=entry[0]
            gene=entry[1]
            if gene not in allgenes: #if a gene is not yet assigned to allgenes, an entry will be created, at it will get the frequency 1 in numgenes
    ...
    def ranker(self,col,lab):
        ...
        data.sort() #sorts the data by case ID because it has index 0 in alldatascored, and then by pedia score, because it has index 1
        data.reverse() #reverses the data so that each scases starts with the entry with the highest pedia score
        ...
        for entry in data:
            ...
            if currcase!=case:
                if not pathoamongdata:
                    combined_rank.append([case, 102]) #if the pathogenic gene had not been in that case anymore the case will be assigned a rank of 105 (so that it is higher than 100 will be regarded as having failed)
