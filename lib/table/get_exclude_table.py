...
FEATURE = ["0", "3", "4", "0_3", "0_4", "3_4",
        ...
        "0_2_3_4",
        "1_2"]
divide_row = {0: 'Photo + Exome + Feature', 6: 'Photo + Exome', 8: 'Exome + Feature', 7: 'Photo + Feature',
        10: 'Photo', 11: 'Exome', 12: 'Feature'}
f_name = {'0':'F', '1':'C', '2':'G', '3':'B', '4':'P'}
divide_idxs = [0, 6, 7, 8, 10, 11, 12]
...
for idx, feature in enumerate(FEATURE):
    ...
    for name in data_type:
        ...
        for j in range(10):
            ...
            for index in range(10):
                filename = cv_dir + "/CV_" + name + "_e_" + feature + "/cv_" + str(j) + "/rank_" + str(index+1) + ".csv"
                top1 = 0
                top10 = 0
                row_count = 0
                total = 0
    ...
    for index in range(len(data_type)):
        perc.append([round(mean(total_count[index]['1']), 2), round(mean(total_count[index]['10']), 2)])
        stds.append([round(mean(total_count[index]['std1']), 2), round(mean(total_count[index]['std10']), 2)])
    if idx in divide_idxs:
        outFile.write("\\multicolumn{5}{|c|}{\\textbf{" + divide_row[idx] + "}}\\\\ \\hline \n")
